...
export async function POST(req: NextRequest, context: any) {
  ...
  try {
    let { page, noPostData = false } = await req.json();
    // console.log(page);
    ...
    if (!userId) {
      return NextResponse.json(
        { success: false, message: "Post is not given. or Not vaild" },
        { status: 400 }
      );
    }
    ...
    let user = await User.findById(userId)
      .populate({
        ...
        select:
          "-updatedAt -createdAt -__v  -userId -productID -isDeleted -verifyTokenExp -verifyToken -forgotPassExp -forgotPassToken -password -reciveRequest -sendRequest -friends -whoSeenProfile -notification",
      })
      .populate({
        path: "reciveRequest",
        // match: { isDeleted: false },
      ...
      .select(
        "-updatedAt -createdAt -__v -verifyTokenExp -verifyToken -forgotPassExp -forgotPassToken -password"
      );
    ...
    if (!user) {
      return NextResponse.json(
        { success: false, message: "User not found with given id." },
        { status: 404 }
      );
    }
    ...
    let posts =
      ...
      (await Post.find({
        ...
        .populate({
          ...
          // match: { isDeleted: false },
          select:
            "-updatedAt -createdAt -__v  -userId -productID -isDeleted -verifyTokenExp -verifyToken -forgotPassExp -forgotPassToken -password -allProfilePic",
        })
        .populate({
          path: "comments",
    ...
    if (
      ...
      if (
        ...
        let getUserFrineds = await User.findById(user._id).populate({
          path: "friends",
          // match: { isDeleted: false },
          select:
            "-updatedAt -createdAt -__v -friendshipRequests -verifyTokenExp -verifyToken -forgotPassExp -forgotPassToken -password -whoSeenProfile -notification",
        });
