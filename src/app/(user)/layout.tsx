...
const LayoutPage = ({
  ...
  const [postionOfFirstLi, setPostionOfFirstLi] = useState<PostitionType>({
    top: "0px",
    left: "0px",
  });
  ...
  const [sliderVisiable, setSliderVisiable] = useState<"hidden" | "inline">(
    "inline"
  );
    ...
    >
      {
        <Navbar
          className={
            dont_show_nav_for_pages.includes(pathname)
              ? " !invisible !-top-10"
              : ""
          }
        />
      ...
      <div className=" relative flex items-start justify-center lg:justify-between lg:px-[10vh] lg:w-[95%] gap-5 flex-col-reverse lg:flex-row">
        ...
        >
          <ul className=" relative flex justify-between  gap-1 lg:block w-full mx-3 sm:mx-8 md:mx-14 lg:mx-0">
            ...
            {tabArr.map((ele) => ele.name).indexOf(pathname.slice(1)) ===
              -1 && (
              <SingleTabLi
                key={0}
                ...
                display={
                  window && window.innerWidth >= LgBreakValue
                    ? "inline-flex"
                    : "none"
                }
                setPostionOfFirstLi={setPostionOfFirstLi}
        ...
        >
          ...
          {pathname === "/home" && <MessageDivForLeptipop />}
        </div>
      </div>
    </div>
  );
...
function SingleTabLi({
  ...
  ele: SingleTabData;
  osmClickHangler: Function;
  setPostionOfFirstLi: Function;
  className?: string;
  display?: string;
}) {
  // const router = useRouter()
  ...
  useEffect(() => {
    if (ele.name === selectedPath && liRef.current) {
      ...
      if (window && window.innerWidth >= LgBreakValue) {
        ...
        let tabNameWidth =
          liRef.current.children[1].getBoundingClientRect().width;
        let tabNameHeight =
          liRef.current.children[1].getBoundingClientRect().height;

        setPostionOfFirstLi({
          top: `${top - 72}px`,
          left: `${-10}px`,
          width: "5px",
          height: `${tabNameHeight * 1.5}px`,
        });
      } else {
        // // // Mobile View here ------------>
        ...
        let tabNameWidth =
          liRef.current.children[1].getBoundingClientRect().width;
        let tabNameLeft =
          liRef.current.children[1].getBoundingClientRect().left;

        setPostionOfFirstLi({
          top: `${-6}px`,
          left: `${tabNameLeft - 15.5}px`,
          width: `${tabNameWidth}px`,
        });
      }
    }
  ...
  return (
    ...
    >
      {ele.name === "profile" ? (
        <>
          {userImg ? (
            <ImageReact
              src={userImg}
              className="h-5 w-5 object-cover rounded-full ring-1 p-[1px] ring-sky-600"
            />
          ) : (
      ...
      <span
        className={` mx-0.5 inline-flex gap-1 justify-center items-center capitalize text-[0.5rem] lg:text-xl leading-[0.8rem] lg:leading-none 
                 ${
                   selectedPath === ele.name &&
                   " font-[cursive] font-bold scale-125 text-sky-600 lg:ml-2.5 "
                 }
      ...
      >
        ...
        {
          // (!userId)
          !session?.user?._id && !publicPages.includes(ele.name) && (
