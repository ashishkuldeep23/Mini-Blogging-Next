    ...
    >
      ...
      <div className=" my-5 border p-2 rounded flex flex-wrap justify-center items-center flex-col">
        ...
        <div className=" text-center">
          <p>Welcome</p>
          <p className=" capitalize text-3xl font-semibold text-cyan-500">
            {userData.username}
          </p>
      ...
      <p>Change Ui of this profile page.</p>
      ...
      <FriendsOfFriendsDiv searchedUser={userData} userData={userData} />
...
function UserProfileImage() {
  ...
  const seeFullSizeHandler = (
    ...
    const innerHtml = (
      <ImageReact src={postImageUrl} className=" rounded max-h-[70vh] " />
    );
  ...
  useEffect(() => {
    (!postImageUrl || postImageUrl !== userData.profilePic) &&
    ...
    <>
      {userData._id && (
        ...
        >
          ...
          <button
            className={`absolute top-0 -left-1  border rounded-full p-1 hover:scale-90 hover:cursor-pointer transition-all ${
              !themeMode ? "bg-black" : "bg-white"
            } active:scale-75 active:opacity-75 transition-all`}
          >
            <MdOutlineZoomOutMap className=" text-3xl" />
          </button>
          ...
          <input
            ...
            id="profile_img"
            onChange={(e) => {
              imgInputOnchangeHandler(e);
            }}
          />

          <label
            htmlFor="profile_img"
            className={`absolute bottom-2 right-2  border rounded-full p-0.5 hover:scale-90 hover:cursor-pointer transition-all ${
              !themeMode ? "bg-black" : "bg-white"
            } active:scale-75 active:opacity-75 transition-all`}
          >
        ...
        <div className="w-40 flex gap-2 justify-center my-2 px-2">
          <button
            className={`px-1  border rounded-full bg-green-500 font-bold text-xs  ${
              userData.profilePic !== postImageUrl ? " scale-100" : "scale-0"
            } transition-all duration-700 `}
            onClick={() => {
              uploadImgaeAndUserDataHandler();
            }}
          >
            Upload
          </button>
...
function AllUploadedPicturesDiv() {
  ...
  function makeDpThisImage(image: string) {
    if (!image)
      return toast.error("Image not getting, please refresh the page.");

    if (image === userData.profilePic)
      return toast.error("This image is already your profile pic.");
  ...
  const seeFullSizeHandler = (e: any, i: number, ele: string) => {
    ...
    const innerHtml = (
      <div className=" flex flex-col items-center justify-center ">
        <ImageReact src={ele} className=" rounded max-h-[70vh] " />
        <button
  ...
  return (
    <div className=" flex flex-col items-center ">
      {userData.allProfilePic && userData.allProfilePic.length > 0 ? (
        <div className=" lg:w-[70%] my-10 rounded-md shadow-xl shadow-cyan-950 ">
          <p className=" text-center underline font-semibold my-2">
            All {userData?.allProfilePic?.length} uploaded picture by you.
          </p>
          <p className=" text-center text-xs">
            Click on image to make profile pic.
          </p>
          ...
          >
            {userData.allProfilePic.map((ele, i) => {
              return (
                <span key={i} className="aspect-square h-[20vh] relative ">
                  {clickedPicIndex === i && (
                    <MainLoader isLoading={isLoading} />
                  )}
                  ...
                  <ImageReact
                    ...
                    className={` aspect-square p-1 w-[20vh] h-[20vh] border rounded-full object-cover active:scale-75 active:opacity-75 hover:cursor-pointer hover:scale-90 transition-all
                                                ...
                                                ${
                                                  userData.profilePic === ele &&
                                                  " border-2 border-green-500 blur-sm"
                                                } 
                                            `}
                    src={ele}
                    alt=""
...
function ReciverRequestDiv() {
  ...
  function acceptFriendRequest(id: string) {
    ...
    if (!session?.user._id)
      return toast.error("You are looking logged out, please login.");
  ...
  return (
    <div className=" w-[96%] sm:w-[65%] md:w-[50%]  px-1 py-2  rounded">
      {
        userData.reciveRequest && userData.reciveRequest.length > 0 && (
          <div className=" w-full flex flex-col items-center">
            <p className=" text-center mb-3 px-2 border-b rounded">
            ...
            {userData.reciveRequest.map((ele, i) => {
              return (
                <div
                  key={ele._id}
                  className={`w-full py-1 border-b flex justify-between px-2 items-center  ${
                    i === 0 && "border-t"
                  } `}
                >
                  <div className=" flex gap-2 items-center ">
                    ...
                    <span className=" ml-7">{ele.username}</span>
                  </div>
    ...
    <div className=" w-[96%] sm:w-[65%] md:w-[50%] px-1 py-2  rounded ">
      {
        userData.sendRequest && userData.sendRequest.length > 0 && (
          <div className=" w-full flex flex-col items-center">
            <p className=" text-center mb-3 px-2 border-b rounded">
              All friend request to user sended by you.
            </p>
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      cancelRequest(ele._id, ele.username);
                    }}
                  >
                    <span>Cancel</span>
                    <span>❌</span>
...
function FriendsOfFriendsDiv({
  ...
  function addFrined() {
    ...
    if (!searchedUser?._id)
      return toast.error("Refresh the page again please.");
    ...
    <>
      ...
      searchedUser.friendsAllFriend.length > 0 ? (
        <Fragment>
          ...
          searchedUser.email === session?.user.email ? (
            // // // if you are a friend of user

            <div
              className={`mt-7 w-11/12 sm:w-3/4 md:w-1/2 px-1 border rounded flex flex-col justify-center items-center 
                                     ${
                                       !seeFriends
              ...
              >
                <div className={`px-0.5 `}>
                  <p className=" text-end">
                    You have{" "}
                    <span>
                      {userData.friendsAllFriend?.length} friend
                      <span>
                        {userData.friendsAllFriend &&
                          userData.friendsAllFriend?.length > 1 &&
                          "s"}
                      </span>
                      .
                    </span>
                ...
                {searchedUser.friendsAllFriend.map((friend, i) => (
                  <SingleUserDiv
                    key={friend._id}
                    i={i}
                    friend={friend}
        ...
        <>
          ...
          searchedUser.reciveRequest
            ...
            <div>
              {searchedUser?._id && searchedUser.email !== userData.email && (
                <button
                  className=" text-2xl px-3 py-1 bg-green-600 text-white rounded-md flex gap-1 items-center hover:scale-110 active:scale-90 transition-all"
                  onClick={() => {
                    addFrined();
...
function SingleUserDiv({
  ...
  return (
    <div
      key={friend._id}
      className={`px-0.5 py-1 border-b flex items-center justify-between flex-wrap gap-1.5 ${
        i === 0 && "border-t"
      } `}
    >
      <div
      ...
      >
        ...
        <div className="ml-5 flex flex-col leading-5">
          ...
          <p className="  text-sm font-extralight">{friend.email}</p>
        </div>
      </div>
      ...
      <div className=" flex gap-2 items-end flex-wrap">
        ...
        <button
          ...
          onClick={() => {
            removeFriend(friend._id, friend.username);
          }}
        >
          <TbUserCancel />
        </button>
...
function AllPostByYou() {
  ...
  return (
    <div className=" my-20 card_container relative sm:px-[8vh] mt-16 flex gap-10 gap-x-64 p-0.5 flex-wrap justify-center items-start ">
      {userData.allPostOfUser.length > 0 && (
        <div>
          <p className=" text-center text-2xl ">All post by you 👇</p>

          <div className=" my-10 card_container relative sm:px-[8vh] mt-16 flex gap-10 gap-x-64 p-0.5 flex-wrap justify-center items-start ">
      ...
      {session?.user?.id &&
        ...
        userData.allPostOfUser.length === 0 && (
          <div className=" text-center">
            ...
            <Link href={"/create"} className=" px-2 text-xs border rounded">
              Write post
            </Link>
          </div>
        )}
