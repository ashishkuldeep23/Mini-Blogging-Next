    ...
    <div className=" relative">
      ...
      <div className="mt-2">
          ...
          <div className=" flex gap-2 flex-wrap justify-between items-center flex-row-reverse ml-auto">
            ...
            <button
              className={`border h-8 w-10 px-1 py-0.5 rounded-lg flex items-center gap-1 ${
                likeIds.includes(userID.toString()) &&
                "text-rose-500 border-rose-500 shadow-md shadow-rose-500"
              } `}
            ...
            <button
              className={`border  h-8 w-10  px-1 py-0.5 rounded-lg flex items-center gap-1
                            ${
                              commentIds.includes(userID.toString()) &&
                              "text-yellow-500 border-yellow-500 shadow-md shadow-yellow-500"
                            }
                        `}
              onClick={commentBtnClicked}
            >
              <span className=" text-[0.6rem] mr-0.5 ">
          ...
          params.includes("post") && (
            <div className="rounded my-1 p-0.5 py-2 flex gap-2 items-center flex-wrap ">
              ...
              <div className=" px-1 flex gap-1 flex-wrap items-center">
                {post?.likesId?.length > 0 && post?.likesId[0] !== "string" && (
                  // post.likesId.map((ele) => ele.)

                  <AnimatedTooltip
                    items={post.likesId.map((ele: any, i) => ({
                      id: i,
                      name: ele.username,
...

        {
          ...
          post?.comments?.length > 0 && params.includes("post") && (
            <AllComments
              post={post}
              updatingComment={updatingComment}
              setUpdatingComment={setUpdatingComment}
...
function PostCommentForm({
  ...
  async function postCommentBtn(
    ...
    if (!commentValue.value)
      return toast.error("Please give comment for this post.");

    if (!checkUserStatus("Please login to give a comment for this post."))
      return;
  ...
  async function updateComment(
    ...
    if (!updatingComment.mode)
      return toast.error("Please click edit icon again.");
      ...
      >
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  if (textAreaInputRef?.current) {
                    textAreaInputRef.current.scrollTop =
                      textAreaInputRef.current?.scrollHeight;
                  }
                  // &&
                }}
              >
                {ele === "X" ? (
                  <span className="px-1 font-bold bg-red-500 text-white ">
                    {ele}
                  </span>
                ) : (
                  ele
        ...
        <div className="flex">
          <textarea
            ...
            onChange={(e) => {
              e.stopPropagation();
              setCommentValue({ value: e.target.value });
            }}
          ></textarea>

          <button
            className={`font-bold border rounded px-2 mx-0.5 ${
              !themeMode ? "bg-green-700 text-white" : "bg-green-300 text-black"
            } `}
            onClick={(e) => {
              e.stopPropagation();
              if (!updatingComment.mode) {
...
function AllComments({
  ...
  return (
    <div className=" my-5 mx-0.5 rounded-md">
      <div className=" flex justify-center">
        ...
        >
          <span className={`${themeMode ? "text-black" : "text-white"}`}>
            ...
            <span className=" relative left-2 top-[5px] inline-flex rotate-[90deg]">
              {" "}
              <PiPaperPlaneRight />{" "}
            </span>
          </span>
...
const SingleCommentUI = ({
  ...
  const commentDoubleClickHandler = (
  ...
  const seeFullSizeHandler = (
    e: any,
    ele: { profilePic: string; _id: string; username: string }
  ) => {
    e?.stopPropagation();

    const innerHtml = (
      <div className=" flex flex-col items-center justify-center ">
        <ImageReact src={ele?.profilePic} className=" rounded max-h-[70vh] " />
        <button
        ...
        >
          See, {ele?.username || "Name Kumar"}'s profile
        </button>
      </div>
    );
...
function SeeMoreOfComment({
  ...
  async function submitReplyHandler() {
    ...
    if (!session?.user?.id)
      return toast.error("Please login to give a comment for this post.");
  ...
  async function commentEditHandler(id: string, index: number) {
    ...
    const response = await fetch(
      `/api/post/comment/reply/${comment._id}/update`,
      option
    );
    let json = await response.json();
  ...
  async function commentDeleteHandler(id: string, index: number) {
    ...
    const response = await fetch(
      `/api/post/comment/reply/${comment._id}/delete`,
      option
    );
    let json = await response.json();
  ...
  const onClickOnReplyHandler = (ele: ReplyInterFace) => {
    ...
    const innerHtml = (
      <div className=" flex flex-col items-center justify-center ">
        ...
        >
          See, {ele?.userId?.username || "Name Kumar"}'s profile
        </button>
      </div>
    );
  ...
  return (
    <>
      <div className="flex flex-col">
          ...
          {(comment.likes > 0 ||
            ...
            <button
              className={`relative mt-0.5  ml-auto mr-0 flex gap-0.5 items-center justify-center border p-0.5 rounded text-[0.6rem] font-semibold font-serif ${
                seeMoreBtn && " bg-yellow-400 text-black "
              } `}
              onClick={(e) => {
                e.stopPropagation();
        ...
        {/* Main UI here -------> */}
        <div
          className={` overflow-hidden relative rounded px-1 py-0.5  flex flex-col ${
            !seeMoreBtn ? " h-1  opacity-100 " : "  h-fit opacity-100"
        ...
        >
          <div className=" py-1  my-3 rounded border-y-2">
              ...
              <div className=" flex flex-wrap items-center">
                {["👍", "😍", "😀", "👌", "👎", "X"].map((ele, i) => {
                  return (
                    <button
                      key={i}
                      className={` overflow-hidden text-xs mx-0.5 my-[2px] border border-l-2 border-b-2  rounded  ${
                        ele === "X" && "ml-3 border-red-500 rounded-xl "
                      } `}
                      onClick={(e) => {
                        e.stopPropagation();
                        if (replyText) {
            ...
            {/* Input for reply -------> */}
            <div className=" flex flex-wrap flex-col sm:flex-row mt-1 w-full">
              <input
              ...
              />
              <button
                className={`font-semibold  ml-auto md:ml-1 border py-0 text-sm rounded px-1 ${
                  themeMode
                        ...
                        session?.user?.email === ele.userId.email && (
                          <div className=" ml-auto border rounded-full px-1 flex items-center gap-1">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                commentDeleteHandler(ele._id, i);
                              }}
                            >
                              <AiTwotoneDelete />
                            </button>
